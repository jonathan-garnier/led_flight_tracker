...
esp_err_t config_storage_save(const device_config_t *config)
{
    ...
    nvs_set_str(handle, "api_user", config->api_username);
    nvs_set_str(handle, "api_pass", config->api_password);
...
esp_err_t config_storage_load(device_config_t *config)
{
    ...
    len = MAX_API_USER_LEN;
    nvs_get_str(handle, "api_user", config->api_username, &len);
    len = MAX_API_PASS_LEN;
    nvs_get_str(handle, "api_pass", config->api_password, &len);
    ...
    ESP_LOGI(TAG, "Config loaded: SSID=%s, API user=%s, bbox=[%.4f,%.4f,%.4f,%.4f]",
             config->ssid, config->api_username, config->lamin, config->lomin, config->lamax, config->lomax);
